...
def generate_email_content(email_to: str, analysis_results: dict, image_bytes: bytes = None,
    ...
    if advanced_cv and advanced_cv.get("horizon"):
        ...
        comp_sub.append({
            ...
            "what_works": "Horizon is perfectly level." if cv_h.get(
                "is_level") else f"Horizon is aligned at {cv_h.get('angle') or 0.0} degrees.",
            "what_could_be_improved": "No alignment adjustment needed." if cv_h.get(
                "is_level") else "Rotate the image slightly to level the horizon line."
        })
    ...
    if advanced_cv and advanced_cv.get("sharpness"):
        ...
        focus_sub.append({
            ...
            "what_works": "Edges are crisp and clear." if cv_s.get("score",
                                                                   75) >= 70 else "Soft details create a gentle transition.",
            "what_could_be_improved": "Focus looks solid." if cv_s.get("score",
                                                                       75) >= 70 else "Increase detail sharpness."
        })
    ...
    post_sub.append({
        ...
        "rating": post_score,
        "what_works": "Minimal editing required." if len(
            suggested_edits) == 0 else f"Only {len(suggested_edits)} tweaks suggested.",
        "what_could_be_improved": f"Apply tweaks: {', '.join(edit_texts)}" if len(
            suggested_edits) > 0 else "No urgent edits."
    })
    if exif_analysis and exif_analysis.get("diagnostics"):
        diag = exif_analysis.get("diagnostics", {})
        post_sub.append({
            ...
            "what_works": "Optimal camera settings selected." if diag.get(
                "status") == "ok" else "Exposure is acceptable.",
    ...
    for cat in categories:
        ...
        imp_list = [s["what_could_be_improved"] for s in cat["sub"] if s.get("what_could_be_improved") and len(
            s["what_could_be_improved"]) > 3 and "No alignment adjustment" not in s[
                        "what_could_be_improved"] and "No urgent edits" not in s["what_could_be_improved"]]
