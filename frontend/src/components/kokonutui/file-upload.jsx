...
const AnimeDiv = ({ initial, animate, exit, transition, children, className, ...props }) => {
  ...
  useLayoutEffect(() => {
    ...
    if (prefersReducedMotion) {
       Object.assign(elRef.current.style, animate);
       return;
    }
    ...
    if (initial) {
      Object.keys(initial).forEach(key => {
        if (key === 'y') elRef.current.style.transform = `translateY(${initial[key]}px)`;
        else if (key === 'scale') elRef.current.style.transform = `scale(${initial[key]})`;
        else elRef.current.style[key] = initial[key];
      });
    }
    ...
    if (animate) {
      const animeProps = {
        duration: transition?.duration ? transition.duration * 1000 : 250,
        easing: 'easeOutExpo', // rule: ease-out
      };
      
      if (animate.y !== undefined) animeProps.translateY = animate.y;
      if (animate.scale !== undefined) animeProps.scale = animate.scale;
      if (animate.opacity !== undefined) animeProps.opacity = animate.opacity;
      
      anime(elRef.current, animeProps);
    }
  }, [animate]); // Re-run if animate props change (like dragging state)
...
export default function FileUpload({
  ...
  return (
    <div
      ...
      role="complementary">
      <div
        className="group relative w-full rounded-xl bg-white p-0.5 ring-1 ring-gray-200 dark:bg-black dark:ring-white/10">
        <div
          className="absolute inset-x-0 -top-px h-px w-full bg-gradient-to-r from-transparent via-blue-500/20 to-transparent" />

        <div
          className="relative w-full rounded-[10px] bg-gray-50/50 p-1.5 dark:bg-white/[0.02]">
          <div
            className={cn(
            ...
            <div
              ...
              <div
                className="absolute inset-[20%] animate-pulse rounded-lg bg-blue-500/5 transition-all duration-300" />
            </div>

            <div
              className="absolute -top-4 -right-4 h-8 w-8 bg-gradient-to-br from-blue-500/20 to-transparent opacity-0 blur-md transition-opacity duration-500 group-hover:opacity-100" />
                ...
                {status === "idle" || status === "dragging" ? (
                  <AnimeDiv
                    ...
                    <div className="mb-8 space-y-4 text-center">
                      <h3
                        className="font-semibold text-gray-900 text-lg tracking-tight dark:text-white">
                        Drag and drop or
                      </h3>
                    ...
                    <p className="mt-8 text-gray-500 text-xs dark:text-gray-400">
